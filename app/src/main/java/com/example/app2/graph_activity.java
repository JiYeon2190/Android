...
public class graph_activity extends Activity {
	...
	private TextView tempavg, tempmax, tempmin;
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		mydb.addValueEventListener(new ValueEventListener() {
			...
			public void onDataChange(@NonNull DataSnapshot snapshot) {
				Double tempAvg = snapshot.child("temp_avg").getValue(Double.class);
				Double tempMax = snapshot.child("temp_max").getValue(Double.class);
				Double tempMin = snapshot.child("temp_min").getValue(Double.class);;

				tempavg.setText(String.format("%.1f", tempAvg) + " °C");
				tempmax.setText(String.format("%.1f", tempMax) + " °C");
				tempmin.setText(String.format("%.1f", tempMin) + " °C");
